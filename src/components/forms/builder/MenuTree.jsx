...
import { ExpandMoreOutlined, MoreVertOutlined, DragIndicatorOutlined } from '@mui/icons-material'

export default function MenuTree({ form, selectedQuestionId, onSelectQuestion }) {
    ...
    <Box sx={{
      ...
      <List aria-label="Form Menu" disablePadding sx={{ flexGrow: 1 }}>
        {(form.sections || []).map((section) => (
          <Box key={section.id}>
            <Accordion elevation={1} expanded={expanded.includes(section.id)} onChange={() => toggleExpanded(section.id)} sx={{ boxShadow: 'none', borderRadius: 0 }}>
              <AccordionSummary
              ...
              <AccordionDetails sx={{ pl: 2, pr: 2, pt: 0.5, pb: 0 }}>
                <List component="div" disablePadding>
                  {(section.items || []).map((sub) => (
                    <Box key={sub.id}>
                      {sub.type === 'subsection' ? (
                        <Accordion elevation={1} defaultExpanded sx={{ boxShadow: 'none', borderRadius: 0 }}>
                          <AccordionSummary
                            expandIcon={<ExpandMoreOutlined />}
